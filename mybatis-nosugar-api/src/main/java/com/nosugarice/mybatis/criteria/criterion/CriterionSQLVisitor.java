...
public interface CriterionSQLVisitor<S> {
     ...
     */
    default void setSQLStrategy(GroupCriterion groupCriterion) {
        for (Criterion criterion : groupCriterion.getCriterions()) {
            if (criterion instanceof GroupCriterion) {
                setSQLStrategy((GroupCriterion) criterion);
            } else if (criterion instanceof ColumnCriterion) {
                criterion.setSqlStrategy((SQLStrategy) () -> visitResultHandle(criterion.accept(CriterionSQLVisitor.this)));
            }
        }
    }
